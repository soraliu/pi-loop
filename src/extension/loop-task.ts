...
import { DEFAULT_EFFORT_LEVEL, resolveEffort } from "../storage/settings.ts";
import { createRunRecord, defaultDataDir, ensureWorkspace } from "../storage/workspace.ts";
 ...
 * 流程：校验入参 → 解析 effort 预设 → 建工作区 → 落 run.json → 返回占位结果。
...
export function runLoopTaskStub(params: LoopToolParams): LoopToolResult {
  ...
  // 档位名解析：缺省回落默认档；非法值由 resolveEffort 抛错（含四合法值提示）
  const level = params.effort ?? DEFAULT_EFFORT_LEVEL;
  const preset = resolveEffort(level);
